...
{
	//不是，我MX_GPIO_Init都配好了为什么我还要再配一次？
...
void display_graphic_16x16(uchar page,uchar column,uchar *dp) //显示16*16的点阵图像、汉字、生僻字或16*16点阵的其他图案
{
	uint i,j;
	for(j=0;j<2;j++)
	{
...
void display_graphic_8x16(uchar page,uchar column,uchar *dp) //显示8*16点阵图像、ASCII、或8*16点阵的自造字符、其他图标
{
	uint i,j;
	for(j=0;j<2;j++)
	{
